...
export const MainForm: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (props.initialValues != null) {
        ...
        const categories = projectNotice.timetables.map(
          (t: Timetable) => t.category as Category
        );
        ...
        const projectCategory = isCategory(props.initialValues.category)
          ...
          : categories.find(
              (c: Category) =>
                c._id === getCategoryId(props.initialValues!.category)
            )!;
        ...
        setSchedule(
          projectNotice.timetables.find(
            (t: Timetable) =>
              (t.category as Category)._id === getCategoryId(projectCategory)
          )!.schedules
        );
        ...
        props.formController.setFieldsValue({
          ...
          secondSemester:
            props.initialValues.secondSemester.map(scheduleAsValue),
        });
      }
  ...
  const populateCategories = useCallback(
    (value: SelectValue) => {
      setCategories(
        notices
          .find((n: Notice) => n._id === value)!
          .timetables.map((t: Timetable) => t.category as Category)
      );
    },
    [notices]
  );
  ...
  const changeSchedule = useCallback(
    (value: SelectValue) => {
      const timetables = notices.find(
        (n: Notice) => n._id === props.formController.getFieldValue("notice")
      )!.timetables;

      setSchedule(
        timetables.find(
          (t: Timetable) => (t.category as Category)._id === value
        )!.schedules
      );
    ...
    <Form name="main" layout="vertical" form={props.formController}>
      <Row gutter={[8, 0]}>
        ...
        {selectedCategory != null &&
          selectedCategory.name !== "Extensão específica do curso" && (
            <>
              <Col span={24}>
                <Form.Item name="secondSemester" label="Horários disponíveis">
