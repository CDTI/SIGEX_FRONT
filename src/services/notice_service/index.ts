...
export const getAllNoticesPaginated = async (params: any) => {
  const response = await httpClient.get("/notices/paginated", {
    params: params,
  });
...
export async function getActiveNoticesForUser(
  id: string,
  withPopulatedRefs: boolean = false
): Promise<Notice[]> {
  let uri = `/notices/activeForUser/${id}`;
  if (withPopulatedRefs) uri += "?withPopulatedRefs=true";
